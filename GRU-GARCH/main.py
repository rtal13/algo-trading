...
TICKER = "EURUSD=X"
PLOT_FOLDER = None
DOWNLOAD_DAYS = 1  # Increased to get more data only can download 5 day max for 11min interval
SEQ_LEN = 26
FUTURE_STEPS = 2
...
def log_epoch_data(file_path, epoch, train_loss, val_loss, train_rmse, val_rmse, val_pip_diff,
    ...
    """
    Log epoch data to a CSV file. If the file doesn't exist or is empty, create it.
    ...
    - train_rmse (float): Training RMSE for the epoch.
    - val_rmse (float): Validation RMSE for the epoch.
    - val_pip_diff (float): Validation mean absolute pip difference for the epoch.
    - train_sharpe (float): Training Sharpe Ratio.
    - val_sharpe (float): Validation Sharpe Ratio.
    - train_profit_factor (float): Training Profit Factor.
    - val_profit_factor (float): Validation Profit Factor.
    - train_max_drawdown (float): Training Max Drawdown.
    - val_max_drawdown (float): Validation Max Drawdown.
    - train_directional_accuracy (float): Training Directional Accuracy (%).
    - val_directional_accuracy (float): Validation Directional Accuracy (%).
    - train_mape (float): Training MAPE.
    ...
    - train_alpha (float): Alpha parameter from the model.
    - train_beta (float): Beta parameter from the model.
    - train_omega (float): Omega parameter from the model.
    """

    # Define the data for the current epoch
    epoch_data = {
        "Epoch": epoch,
    ...
    # Try to read the existing file, or create a new one if it doesn't exist or is empty
    try:
        existing_df = pd.read_csv(file_path)
        # Concatenate the new data with the existing data
        updated_df = pd.concat([existing_df, epoch_df], ignore_index=True)
    except (FileNotFoundError, pd.errors.EmptyDataError):
        # If the file doesn't exist or is empty, use the new DataFrame
        updated_df = epoch_df

    # Save the updated DataFrame to the file
    updated_df.to_csv(file_path, index=False)
    print(f"Epoch {epoch} data logged to {file_path}.")
